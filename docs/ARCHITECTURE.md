...
Aeternum Ally is a multi-tenant SaaS web application that helps SMEs produce ESRS/CSRD-compliant sustainability reports. The system sits at the intersection of three external services: a relational database with built-in auth (Supabase), a generative AI API (Google Gemini), and a static hosting + serverless runtime (Netlify).
...
| Array data (assessments, KPIs) | Explicit CRUD via `dbService.ts` wrapper functions |
...
```
components/   One file per feature screen (AuthScreen, AssessmentForm, …)
...
|---|---|---|
| `api.ts` | Proxies Gemini calls; logs token usage to `ai_usage_log` | Bearer JWT (any org member) |
