...
The AI layer (Google Gemini) is an accelerator, not an authority. Every AI-generated suggestion — impact descriptions, financial risk narratives, KPI proposals, SWOT entries — is editable before saving. The system never commits AI output directly to the database without user review.
...
- The SWOT wizard is step-by-step (one quadrant at a time) rather than a single dense form.
...
**In practice:**
- Generated statements are presented as drafts for review, not final reports.
