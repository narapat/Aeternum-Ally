...
AeternumAlly is a multi-tenant SaaS web application that helps SMEs produce ESRS/CSRD-compliant sustainability reports. The system sits at the intersection of three external services: a relational database with built-in auth (Supabase), a generative AI API (Google Gemini), and a static hosting + serverless runtime (Netlify).
